...
module.exports = {
  ...
  aliases: ["screenshot"],
  category: "search",
  description: "Mengambil tangkapan layar dari halaman web mana pun.",
  ...
  run: async (client, message, args) => {
    ...
    if (urls.length < 8)
      return message
        .reply(
          "<a:failed:717824012184715336> https is too short to reach - 8 limit"
        )
    ...
    try {
      ...
      if (err.status === 404)
        return message.channel
          .send("Tidak dapat menemukan hasil apa pun. URL tidak valid?")
          .then(m => m.delete({ timeout: 14000 }).catch(e => {}));
      return message
        .reply(`Oh no, an error occurred: \`${err.message}\`. Coba lagi nanti!`)
